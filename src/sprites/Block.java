...
public class Block implements Collidable, Sprite, HitNotifier {
     ...
     */
    public Block(Rectangle rectangle, Fill[] fills, Color stroke, int hits) {
     ...
     * @param hits      The block hits until it disappears.
     */
    public Block(Rectangle rectangle, Fill fill, Color stroke, int hits) {
    ...
    public Velocity hit(Ball hitter, Point collisionPoint, Velocity currentVelocity) {
        ...
        if (collisionPoint.isOnHorizontalLine(this.geoRep.getUpperEdge()) || collisionPoint.isOnHorizontalLine(
                this.geoRep.getLowerEdge())) {
            return new Velocity(currentVelocity.getDX(), -currentVelocity.getDY());
        }
        if (collisionPoint.isOnVerticalLine(this.geoRep.getLeftEdge()) || collisionPoint.isOnVerticalLine(
    ...
    /**
     * change the upper left point of the block.
     *
     * @param point the new upper left.
     */
    public void changeUpperLeft(Point point) {
        this.geoRep = new Rectangle(point, geoRep.getWidth(), geoRep.getHeight());
    }
    /**
    ...
    private void drawStroke(DrawSurface d, Point upperLeft) {
        ...
        d.drawRectangle((int) upperLeft.getX(), (int) upperLeft.getY(), (int) this.geoRep.getWidth(),
    ...
    private void drawFill(DrawSurface d, Point upperLeft) {
        ...
        if (fill.isImage()) {
            ...
            d.fillRectangle((int) upperLeft.getX(), (int) upperLeft.getY(), (int) this.geoRep.getWidth(),
                            (int) this.geoRep.getHeight());
        }
    }
    /**
    ...
    public void drawHitsIndicator(DrawSurface d) {
        ...
        if (this.hitPoints == 0) {
            d.drawText((int) this.getMiddlePoint().getX(), (int) this.getMiddlePoint().getY(), "X", 8);
        } else {
            d.drawText((int) this.getMiddlePoint().getX(), (int) this.getMiddlePoint().getY(),
     ...
     */
    public void removeHitListeners() {
     ...
     * Move block down.
     *
     * @param yCoorChange The Y coordinate change.
     */
    public void moveDown(double yCoorChange) {
        double y = this.geoRep.getUpperLeft().getY();
        this.geoRep.setYUpperLeft(y + yCoorChange);
    }
}
